/* ================================================================
   jscripVistaCliente.js  –  Vista Cliente (menú + carrito)
   GET  /api/productos
...
async function verificarSesionCliente() {
  try {
    ...
    if (el) el.textContent = "👋 Hola, " + (data.nombre?.split(" ")[0] || "Cliente");
  } catch { redirigir(); }
}
...
async function cargarProductos(filtro = "all") {
  try {
    ...
    if (filtro !== "all") url += "?categoria=" + filtro;
...
function agregarDesdeCard(e, id) {
  ...
  if (prod && validarStock(prod)) insertarAlCarrito(prod);
}
