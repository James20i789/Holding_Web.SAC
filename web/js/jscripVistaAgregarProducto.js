    ...
    document.querySelectorAll(".sidebar .nav-link").forEach(link => {
        link.addEventListener("click", e => {
            ...
            if (link.dataset.section === "ventas")
                cargarVentas();
            if (link.dataset.section === "estadisticas")
                cargarEstadisticas();
        });
    });
});
...
async function verificarSesionAdmin() {
    try {
        ...
        if (el)
            el.textContent = data.nombre || "Admin";
    } catch {
        window.location.href = BASE + "/pages/index.html";
    }
}
...
function renderProductos() {
    ...
    if (cat)
        lista = lista.filter(p => String(p.idCategoria) === String(cat));
...
async function guardarProducto() {
    ...
    if (!nombre)
        return Swal.fire("Falta nombre", "Ingresa el nombre del producto", "warning");
    if (!idCategoria)
        return Swal.fire("Falta categoría", "Selecciona una categoría", "warning");
    ...
    if (fileInput?.files.length > 0)
        fd.append("imagenFile", fileInput.files[0]);
...
async function cargarEstadisticas() {
    try {
        ...
        if (el("statProductos"))
            el("statProductos").textContent = Array.isArray(prods) ? prods.length : 0;
        if (el("statCategorias"))
            el("statCategorias").textContent = Array.isArray(cats) ? cats.length : 0;
        if (el("statIngresosVentas"))
            el("statIngresosVentas").textContent = "S/ " + Number(ingresos?.total || 0).toFixed(2);
        if (el("statValor")) {
